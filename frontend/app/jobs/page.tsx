...
import { getScanJobs, getScanStatus, retryScanErrors, deleteScanJob, getRagJobs, triggerRagIndex } from '@/lib/api';
import type { ScanJob, ScanSummary, RagJobStatus } from '@/lib/types';

type Filter = 'all' | 'active' | 'error';
type JobType = 'scan' | 'rag';
...
function statusBadge(status: string) {
    ...
    switch (status) {
        ...
        case 'processing':
        case 'chunking':
            return <span className={`${base} bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300`}>{status}</span>;
        case 'embedding':
            return <span className={`${base} bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300`}>embedding</span>;
        case 'done':
...
const RAG_ACTIVE_STATUSES = new Set(['pending', 'chunking', 'embedding']);
...
export default function JobsPage() {
    ...
    // RAG state
    const [ragJobs, setRagJobs] = useState<RagJobStatus[]>([]);
    const [ragFilter, setRagFilter] = useState<Filter>('active');
    const [ragLoading, setRagLoading] = useState(true);
    const [ragRetryState, setRagRetryState] = useState<'idle' | 'loading' | 'done'>('idle');
    ...
    const loadRag = useCallback(async () => {
        setRagLoading(true);
        try {
            const statusParam = ragFilter === 'error' ? 'error' : undefined;
            const data = await getRagJobs({ status: statusParam });
            setRagJobs(data);
        } finally {
            setRagLoading(false);
        }
    ...
    useEffect(() => { if (jobType === 'scan') loadScan(); }, [loadScan, jobType]);
    useEffect(() => { if (jobType === 'rag') loadRag(); }, [loadRag, jobType]);
    ...
    const handleRagRetry = async () => {
        setRagRetryState('loading');
        try {
            await triggerRagIndex();
            setRagRetryState('done');
            await loadRag();
        } finally {
            setTimeout(() => setRagRetryState('idle'), 2000);
        }
    };

    const visibleRagJobs = ragFilter === 'active'
        ? ragJobs.filter(j => RAG_ACTIVE_STATUSES.has(j.status))
        : ragJobs;

    const ragSummary = {
        pending: ragJobs.filter(j => j.status === 'pending').length,
        inProgress: ragJobs.filter(j => j.status === 'chunking' || j.status === 'embedding').length,
        done: ragJobs.filter(j => j.status === 'done').length,
        error: ragJobs.filter(j => j.status === 'error').length,
    };

    const hasRagErrors = ragSummary.error > 0;
    const hasScanErrors = summary && summary.error > 0;
        ...
        <div className="flex flex-1 flex-col">
            ...
            <div className="sticky top-0 z-10 flex h-14 shrink-0 items-center justify-between border-b border-zinc-200 px-6 dark:border-zinc-800 dark:bg-zinc-950">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {jobType === 'rag' && hasRagErrors && (
                        <button
                            onClick={handleRagRetry}
                            disabled={ragRetryState === 'loading'}
                            className="flex items-center gap-1.5 rounded px-2.5 py-1.5 text-xs font-medium text-red-600 transition-colors hover:bg-red-50 disabled:opacity-50 dark:text-red-400 dark:hover:bg-red-900/20"
                        >
                            {ragRetryState === 'loading' ? (
                                <svg className="h-3.5 w-3.5 animate-spin" fill="none" viewBox="0 0 24 24">
                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                                </svg>
                            ...
                            {ragRetryState === 'done' ? 'Requeued' : 'Retry errors'}
                        </button>
                    )}
                    <button
                        onClick={jobType === 'scan' ? loadScan : loadRag}
                        disabled={jobType === 'scan' ? loading : ragLoading}
                    ...
                    >
                        <svg className={`h-3.5 w-3.5 ${(jobType === 'scan' ? loading : ragLoading) ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                        </svg>
                        Refresh
                    </button>
            ...
            <div className="flex flex-col gap-4 p-6">
                {/* Job type selector */}
                <div className="flex gap-1 border-b border-zinc-200 dark:border-zinc-800 pb-3">
                    {(['scan', 'rag'] as JobType[]).map((t) => (
                        <button
                            key={t}
                            onClick={() => setJobType(t)}
                            className={`rounded px-3 py-1.5 text-xs font-medium transition-colors uppercase tracking-wide ${
                                jobType === t
                                    ? 'bg-zinc-900 text-zinc-50 dark:bg-zinc-50 dark:text-zinc-900'
                                    : 'text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-200'
                            }`}
                        >
                            {t === 'scan' ? 'Scan' : 'RAG Index'}
                        </button>
                    ))}
                </div>
                ...
                {jobType === 'scan' ? (
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-4 gap-3">
                            {[
                                { label: 'Pending', value: ragSummary.pending, color: 'text-zinc-600 dark:text-zinc-400' },
                                { label: 'In Progress', value: ragSummary.inProgress, color: 'text-blue-600 dark:text-blue-400' },
                                { label: 'Done', value: ragSummary.done, color: 'text-green-600 dark:text-green-400' },
                                { label: 'Errors', value: ragSummary.error, color: 'text-red-600 dark:text-red-400' },
                            ].map(({ label, value, color }) => (
                                <div key={label} className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
                                    <p className="text-xs text-zinc-500 dark:text-zinc-400">{label}</p>
                                    <p className={`mt-1 text-2xl font-semibold tabular-nums ${color}`}>{value}</p>
                        ...
                        ) : visibleRagJobs.length === 0 ? (
                            <div className="flex flex-1 items-center justify-center">
                                <p className="text-sm text-zinc-400 dark:text-zinc-500">
                                    {ragFilter === 'all'
                                        ? 'No RAG index jobs yet — trigger indexing from Settings.'
                                        : `No ${ragFilter} RAG jobs.`}
                                </p>
                            </div>
                        ) : (
                            <div className="overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-800">
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900">
                                            <th className="px-4 py-2.5 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400">Video</th>
                                            <th className="px-4 py-2.5 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400">Status</th>
                                            <th className="px-4 py-2.5 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400">Created</th>
                                            <th className="px-4 py-2.5 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400">Duration</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                                        {visibleRagJobs.map((job) => (
                                            <Fragment key={job.id}>
                                                <tr className="bg-white dark:bg-zinc-950">
                                                    <td className="max-w-xs px-4 py-3">
                                                        <span className="block truncate font-medium text-zinc-900 dark:text-zinc-50">
                                                            {job.video_title ?? job.video_filename ?? `Video #${job.video_id}`}
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-3">{statusBadge(job.status)}</td>
                                                    <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400">{formatTime(job.created_at)}</td>
                                                    <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400">{formatDuration(job.created_at, job.embedded_at)}</td>
                                                </tr>
                                                {job.status === 'error' && job.error_message && (
                                                    <tr className="bg-red-50 dark:bg-red-950/20">
